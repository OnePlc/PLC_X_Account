<?php
/**
 * ApiController.php - Account Api Controller
 *
 * Main Controller for Account Api
 *
 * @category Controller
 * @package Application
...
class ApiController extends CoreController {
    ...
    public function indexAction() {
        ...
        $aReturn = ['state'=>'success','message'=>'Welcome to onePlace Account API'];
    ...
    public function listAction() {
        ...
        # Check license
        if(!$this->checkLicense('account')) {
            $aReturn = ['state'=>'error','message'=>'no valid license for account found'];
            echo json_encode($aReturn);
            return false;
        }
        ...
        foreach($aLangs as $sLoadLang) {
            if(file_exists(__DIR__.'/../../../oneplace-translation/language/'.$sLoadLang.'.mo')) {
                $translator->addTranslationFile('gettext', __DIR__.'/../../../oneplace-translation/language/'.$sLang.'.mo', 'account', $sLoadLang);
            }
        }
        ...
        $aFields = $this->getFormFields('account-single');
        $aFieldsByKey = [];
        ...
        # Get All Account Entities from Database
                        ...
                        switch($oField->type) {
                            case 'multiselect':
                                ...
                                foreach($oTags as $oTag) {
                                    $aTags[] = ['id'=>$oTag->id,'label'=>$translator->translate($oTag->text,'account',$sLang)];
                                }
                            ...
                            case 'select':
                                ...
                                if($oTag) {
                                    if (property_exists($oTag, 'tag_value')) {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->id, 'label' => $translator->translate($oTag->tag_value,'account',$sLang)];
                                    } else {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->getID(), 'label' => $translator->translate($oTag->getLabel(),'account',$sLang)];
                                    }
                                }
                                break;
                            case 'text':
                            case 'date':
                            case 'textarea':
                                $aPublicItem[$oField->fieldkey] = $translator->translate($oItem->getTextField($oField->fieldkey),'account',$sLang);
                                break;
                            default:
                                break;
                        }
    ...
    /**
     * Get a single Entity of Account
     *
     * @return bool - no View File
     * @since 1.0.0
     */
    public function getAction() {
        ...
        try {
            ...
            $aReturn = ['state'=>'error','message'=>'Account not found','oItem'=>[]];
            echo json_encode($aReturn);
            return false;
        }

        # Print Entity
        $aReturn = ['state'=>'success','message'=>'Account found','oItem'=>$oItem];
        echo json_encode($aReturn);
